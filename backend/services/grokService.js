...
export async function analyzeImageVisualsGrok(buffer, mimeType) {
  ...
  try {
    ...
    console.log('[GrokService] Sending image analysis request to xAI...');
...
Return JSON:
{
  ...
  recommendations: Array<string>
    ...
    if (!response.ok) {
      ...
      throw new Error(`xAI API HTTP ${response.status}: ${errText.slice(0, 200)}`);
    }

    const data = await response.json();
    const messageContent = data.choices?.[0]?.message?.content;
    if (!messageContent) {
      console.warn('[GrokService] Empty response content from xAI.');
      return EMPTY;
    }
